...
class IPythonSession:
    ...
    def _setup_fins_environment(self):
        ...
        try:
            ...
            print(
                f"FINS environment loaded in IPython session {self.session_id}")
        ...
        try:
            with redirect_stdout(stdout_capture), redirect_stderr(stderr_capture):
                ...
                if not success and result.error_in_exec:
                    ...
                    # For user-facing exceptions, show clean error without stack trace
                    from fins.exceptions import SymbolNotFound
                    if isinstance(exc, (SymbolNotFound, ValueError)):
                        error = f"{exc_type}: {str(exc)}"
                    else:
                ...
                print(
                    f"[DEBUG] Rich element cache size: {len(self.rich_element_cache)}")
                if self.rich_element_cache:
                    print(
                        f"[DEBUG] Rich elements captured: {list(self.rich_element_cache.keys())}")
    ...
    def _register_display_hook(self):
        ...
        def custom_publish(data, metadata=None, source=None, **kwargs):
            ...
            if isinstance(data, Figure):
                ...
                print(
                    f"[DEBUG] Captured matplotlib figure as rich element: {element_id}")
